...
- Search existing issues and pull requests to avoid duplicating work. For substantial or breaking changes, open an issue first so we can agree on scope.
...
- Optional tools used by CI (installed automatically in workflows but helpful locally): `golangci-lint` and other tooling you can install with `make -C .github update-linters`.

## 3. Workflow and Branching

1. Fork the repository and create topic branches from `main` (for example `feat/support-encoding`, `docs/security-refresh`).
...
- Sign your commits using SSH or GPG. Use `git commit -S` to do it automatically.
...
5. Watch the CI status and address any failures appropriately.
...
- Architecture and guidelines: [docs/architecture_and_guidelines.md](../docs/architecture_and_guidelines.md)
- Security threat model and assurance case: [docs/security_model.md](../docs/security_model.md)
- Governance model: [docs/governance.md](../docs/governance.md)
- Roadmap and open initiatives: [docs/roadmap.md](../docs/roadmap.md)

Thank you for helping keep this project reliable and secure!
